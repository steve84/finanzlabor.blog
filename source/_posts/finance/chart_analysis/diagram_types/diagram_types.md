...
p: finance/chart_analysis/diagram_types/diagram_types.md
...
Dieser Artikel (kursiv dargestellt) ist Teil der Serie "Technische Analyse", welche aus folgenden Teilen besteht (bereits erschienene Beiträge sind via Link in der Aufzählung erreichbar):
...
Das Linien-Diagramm ist eines der bekanntesten Diagramm-Typen überhaupt. Dabei werden die abgetragenen Datenpunkte durch gerade Linien miteinander verbunden. Damit werden normalerweise die Schlusskurse eines Wertpapiers visualisiert. Auf dem Beispiel-Diagramm sind die täglichen Schlusskurse seit Jahresbeginn (YTD) eingezeichnet. Die Grafik stellt die beiden aktuellsten Trend-Phasen (Beginn der Corona-Krise sowie die anschliessende Erholung) dar.

![MSIC ACWI ETF, Typ: Linie (Line), Periode: 1 Tag](line_chart_1d_ACWI.png)

### Kerzen (Candlestick)

Dieser Diagrammtyp besteht aus einer Reihe von Kerzen, welche aus einem Kerzenkörper, einem oberen (auch Docht genannt) sowie unteren Schatten (Lunte) zusammengesetzt werden. Der Kerzenkörper (engl. body) zeigt den Eröffnungskurs sowie den Schlusskurs an. Ist dieser grün (bzw. weiss), war dieser am Ende der Beobachtung höher als zu Beginn. Ein roter (bzw. schwarzer) Körper zeigt das Gegenteil an. Das Ende des Kerzendochts (engl. shadows oder wicks) stellt den höchsten, die Lunte den tiefsten Wert innerhalb einer Beobachtung dar. Im unten abgebildeten Kerzen-Chart zum ACWI ETF sind nicht nur die Schlusskurse, sondern auch die Geschichte der Entstehung derjenigen, ersichtlich. Die Entstehung einer Lücke (Gap), welche in einem späteren Beitrag dieser Serie vorgestellt wird, ist in einem Linien-Diagramm nicht sichtbar. Diese, am 11. Juni 2020 entstandene Gegebenheit, ist für manche Chart-Techniker ein wichtiger Hinweis für den weiteren Kursverlauf.

![MSIC ACWI ETF, Typ: Kerzen (Candlestick), Periode: 1 Tag](candle_chart_1d_ACWI.png)

Es existieren Ableitungen wie zum Beispiel die Heikin-Ashi-Kerzen[^1], welche mit Hilfe der OHLC-Werte ($P_O$, $P_H$, $P_L$ und $P_C$) - des vorhergehenden $t-1$, sowie aktuellen Tages $t$ - berechnet werden:
...
Dabei handelt es sich um eine sehr einfache Form der Berechnung, es existieren noch weit komplexere Formeln für die Konstruktion des Kerzentyps. So könnte zum Beispiel das geometrische anstelle des arithmetischen Mittels verwendet werden, um den Schlusskurs zu ermitteln. Durch die Glättung dieser Werte wirkt die Kerzenabfolge harmonischer. Damit werden zum Beispiel rote "Störkerzen" in einem positiven Trend durch grüne Kerzen ersetzt.

![MSIC ACWI ETF, Typ: Heikin-Ashi-Kerzen, Periode: 1 Tag](heikin_ashi_candle_chart_1d_ACWI.png)

### Balken (Bar)
Diese Art von Diagrammen hat im Gegensatz zu den Kerzen keinen ausgefüllten Körper, sondern markiert Eröffnung (links) und Schluss (rechts) mit einem simplen Querstrich. Diese schlankere Abwandlung der Kerzencharts ist für die Kombination mit anderen Diagramm-Typen (zum Beispiel eine Überlagerung des gleitenden Durchschnitts in Form einer Linie) ideal.

![MSIC ACWI ETF, Typ: Balken (Bar), Periode: 1 Tag](bar_chart_1d_ACWI.png)

### Renko
Dieser Chart-Typ zeigt nur einen weiteren Klotz (engl. brick) an, wenn der Kurs eine vordefinierte Veränderung erfährt. Die Zeitachse hat daher keinen linearen Verlauf wie die bis hierhin vorgestellten Typen. Die einzelnen Elemente haben alle die gleiche Länge und Höhe, sie unterscheiden sich nur durch ihre Farbe (grün: positive Bewegung, rot: negative Bewegung) und Position (jedes Element schliesst direkt an der oberen oder unteren Ecke seines Vorgängers an). Lange Seitwärtsbewegungen ohne grosse Schwankungen sind daher nicht auf dem Diagramm ersichtlich. Um von einer Aufwärtsbewegung in eine Abwärtsbewegung (oder umgekehrt) zu wechseln, muss der Preis sich um mindestens das Zweifache der vordefinierten Veränderung bewegen. In der, mit Hilfe von TradingView[^2] erstellten Grafik, sind die beiden aktuellsten Trendphasen sehr gut ersichtlich. Es wurde ein Betrag von 3 USD als Blockhöhe festgelegt, daher braucht es eine Gegenbewegung von mindestens 6 USD, um einen Farbwechsel zu bewirken. Das Renko-Diagramm ist perfekt dazu geeignet, um primäre Trendphasen zu visualisieren. Die Striche (Schatten) unter- oder oberhalb der Renko-Bauklötze zeigen an, ob in der Zeitdauer zwischen aktuellem und nachfolgendem Block ein Preis, welcher eine Trendwende herbeigeführt hätte, existiert hat. Da es sich dabei nicht um den Schlusskurs handelte, wurde kein neuer Block in entgegengesetzter Farbe an die bestehende Kette angehängt. Diese Hilfslinien treten meistens kurz vor und kurz nach einer Trendumkehrung auf.
...
Die Unterstützungs- beziehungsweise Widerstandslinie ist eine horizontale Gerade, an welcher Preisbewegungen mehrfach abprallen und sich nach der Berührung in die gegen gesetzte Richtung weiter bewegen. Je häufiger eine solche Linie getestet (touchiert) wird, umso signifikanter wird diese. Wird diese imaginäre Marke nach mehrmaligem testen durchbrochen, findet ein Rollenwechsel statt. Dabei wird ein Widerstand zu einer Unterstützung und umgekehrt. Dieser Rollentausch ist im Kerzen-Diagramm der Unilever-Aktie gegen Ende von Q1 2019 gut ersichtlich.

![Unilever NV, Typ: Kerzen (Candlestick), Periode: 1 Tag](candle_chart_1d_support_resistance_UNA.AS.png)

Das Zeichnen einer solchen Geraden ist keine exakte Wissenschaft und es gibt mehrere Vorgehensweisen um diese zu erstellen. Mehrheitlich werden die beiden Enden der Preisentwicklung innerhalb einer Zeitperiode verwendet. Dazu wird die Linie mit Hilfe eines Kerzen-Charts bestmöglich an die Enden der einzelnen Kerzendochte (oder Lunten) konstruiert. Eine gewisse Ungenauigkeit muss jedoch in Kauf genommen werden, da nicht jeder Docht (jede Lunte) exakt auf der gezeichneten Linie enden wird. Dieses Problem ist auf dem Kerzendiagramm der Nestlé-Aktie gut ersichtlich. Die eingezeichnete Widerstandslinie wird fünfmal getestet und wird im ersten Quartal 2019 durchbrochen. Bei den Schnittpunkten 2 bis 4 endet die Verlängerung der Kerze fast exakt auf der gezogenen Geraden. Der obere Docht der sechsten Kerze (26.06.2017) endet mit einem deutlich grösseren Abstand als seine Nachfolger.

![Nestlé SA, Typ: Kerzen (Candlestick), Periode: 1 Tag](candle_chart_5d_support_resistance_NESN.SW.png)

In der Nähe solcher Linien ist häufig ein erhöhtes Handelsvolumen zu beobachten. Die Wahrscheinlichkeit, dass sich der Preis in der nahen Zukunft weiter von der berührten Linie wegbewegt (abprallt), steigt. Oft entstehen die Marken bei runden Zahlen, da sich die menschliche Psyche damit wohler fühlt. Eine weitere Gegebenheit - welche dieses Phänomen noch verstärkt - ist, dass manche Broker exotische Beträge automatisch auf die nächste Zwischenstufe auf- (z.B. in Stufen von 5 Rappen beziehungsweise Cents) oder abrunden. Dies verhindert die Erstellung einer Order mit Preisen in den Zwischenbereichen.
...
[^2]: [TradingView, webbasierte Charting Software](https://www.tradingview.com)
